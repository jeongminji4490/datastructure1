 ...
 *
 */
//test case-1: insert 4 2 5 1 3 6 
//test case-2: insert 30 20 50 10 15
//test case-3: insert 30 20 50 40 60 35 42 55 70 65
	...
	while (cNode != NULL) //cNode가 가리키는 노드가 존재한다면
	{
		if (key == cNode->key) //삭제하려는 노드의 key가 cNode의 key와 같다면
		{
			...
			else if (cNode->left == NULL || cNode->right == NULL)
			{
				...
				{
					if(pNode->key<cNode->right->key)
					pNode->right=cNode->right;
					else
					pNode->left = cNode->right; //pNode의 오른쪽 링크가 cNode의 오른쪽 자식 노드를 가리키게 함
				}
				free(cNode); //cNode의 메모리 해제
				return 0;
			}
