...
int selectionSort(int *a)
{
	...
	for (i = 0; i < MAX_ARRAY_SIZE; i++) //MAX_ARRAY_SIZE만큼 반복
	{
		...
		for(j = i+1; j < MAX_ARRAY_SIZE; j++) //MAX_ARRAY_SIZE만큼 반복, i의 다음값부터 시작
		{
...
int insertionSort(int *a)
{
	...
	for(i = 1; i < MAX_ARRAY_SIZE; i++) //MAX_ARRAY_SIZE만큼 반복
	{
...
int bubbleSort(int *a)
{
	...
	for(i = 0; i < MAX_ARRAY_SIZE; i++) //MAX_ARRAY_SIZE만큼 반복
	{
		for (j = 0; j < MAX_ARRAY_SIZE; j++) //MAX_ARRAY_SIZE만큼 반복
		{
			if (a[j-1] > a[j]) //j-1번째 배열의 값이 j번째 배열의 값보다 크다면
			{
        //swap//
				t = a[j-1]; //t에 j-1번째 배열의 값 저장
				a[j-1] = a[j]; //j-1번째 배열에 j번째 배열의 값 저장
				a[j] = t; //j번째 배열에 t저장
			}
		}
	}
...
int shellSort(int *a)
{
	...
	for (h = MAX_ARRAY_SIZE/2; h > 0; h /= 2) //h=MAX_ARRAY_SIZE에서 0이 될 때까지 h를 반으로 나누면서 반복
	{
		for (i = 0; i < h; i++) //h만큼 반복
		{
			for(j = i + h; j < MAX_ARRAY_SIZE; j += h)
      //j의 값을 i에 h를 더한 값으로 지정, j에 h를 더하면서 MAX_ARRAY_SIZE만큼 반복함
			{
